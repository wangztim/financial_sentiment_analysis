...
def parse_csv(file): return pd.read_csv(file, parse_dates=[
    'created_at'], converters={'symbols': eval})


messages = pd.concat((parse_csv(f)
                      for f in all_csv_files), ignore_index=True, sort=False)
...
def filter_urls(text): return re.sub(r"http\S+", "", str(text))
